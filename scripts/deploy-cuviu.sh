...
LOCAL_SUPPORT_FILE="$ROOT_DIR/support.html"
...
REMOTE_SUPPORT_FILE="$REMOTE_DIR/support.html"
REMOTE_SUPPORT_TMP="$REMOTE_DIR/support.html.tmp"
...
if [[ -f "$LOCAL_SUPPORT_FILE" ]]; then
  echo "Deploying $LOCAL_SUPPORT_FILE"
  echo "  -> $SSH_USER@$SSH_HOST:$REMOTE_SUPPORT_FILE"
  scp -i "$SSH_KEY" -P "$SSH_PORT" "$LOCAL_SUPPORT_FILE" "$SSH_USER@$SSH_HOST:$REMOTE_SUPPORT_TMP"
  ssh -i "$SSH_KEY" -p "$SSH_PORT" "$SSH_USER@$SSH_HOST" "mv '$REMOTE_SUPPORT_TMP' '$REMOTE_SUPPORT_FILE' && chmod 644 '$REMOTE_SUPPORT_FILE'"
fi
